...
Solution::Solution(Input *input)
{
    ...
    capacityVehicle_ = vector<double>(in->numVehicle(), in->capacity());
    tour_.resize(in->numVehicle());
    tourDistance_ = vector<double>(in->numVehicle(), 0.0);
    ...
    {
        location.resize(0);
        for (int i = 1; i < in->numLocation(); i++)
            location.push_back(i);

        shuffle(location.begin(), location.end(), default_random_engine(time(0)));

        for (int v = 0; v < in->numVehicle(); v++)
        {
            tour_[v] = {0};
            capacityVehicle_[v] = in->capacity();
        }

        for (int v = 0; v < in->numVehicle() && location.size(); v++)
        {
